...
const BladeShapeTypeListPage = () => {
    ...
    const fetchData = useCallback(async () => {
        try {
            setLoading(true);
            const data = await bladeShapeTypeService.getAll();
            setBladeShapeTypes(data);
        } catch (error) {
    ...
    const filteredAndSortedData = useMemo(() => {
        const filtered = bladeShapeTypes.filter((item) =>
        ...
        return filtered;
    }, [bladeShapeTypes, searchTerm, sortDirection]);
    ...
    const handleDelete = async (id: string) => {
        if (!window.confirm("Ви впевнені, що хочете видалити цей тип?")) return;
        try {
            await bladeShapeTypeService.delete(id);
            setBladeShapeTypes((prev) => prev.filter((item) => item.id !== id));
        } catch (error) {
                        ...
                        <div className="flex items-center space-x-4">
                            ...
                            <div>
                                ...
                                <p className="text-[#2d3748]/60">
                                    Управління типами для форм леза
                                </p>
                            </div>
                        </div>
                        <button
                            onClick={() => router.push("/blade-shape-type/0")}
                                ...
                                >
                                    ...
                                    <td className="p-4">
                                        <div className="flex items-center justify-center space-x-2">
                                            <button
                                                onClick={() =>
                                                    router.push(`/blade-shape-type/${item.id}`)
                                                }
...
export default BladeShapeTypeListPage;