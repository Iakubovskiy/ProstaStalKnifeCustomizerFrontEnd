...
const ProductTagListPage = () => {
  ...
  const filteredAndSortedData = useMemo(() => {
    let filtered = productTags.filter((item) =>
    ...
    return filtered;
  }, [productTags, searchTerm, sortField, sortDirection, locale]);
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      setProductTags((prev) => prev.filter((item) => item.id !== id));
    } catch (error) {
            ...
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-[#2d3748]/60">
                  Управління тегами для каталогу
                </p>
              </div>
            </div>
            <button
              onClick={() => router.push("/product-tag/0")}
                  ...
                  >
                    ...
                    <td className="p-4">
                      <div className="flex items-center justify-center space-x-2">
                        <button
                          onClick={() => router.push(`/product-tag/${item.id}`)}
...
export default ProductTagListPage;
