...
const SheathListPage = () => {
    ...
    const fetchData = useCallback(async () => {
        try {
            setLoading(true);
            const [sheathsData, typesData] = await Promise.all([
    ...
    }, [sheathService, bladeShapeTypeService]);
    ...
    const filteredAndSortedData = useMemo(() => {
        const filtered = sheaths.filter((item) => {
            ...
            const typeMatch = typeFilter === "all" || item.type.id === typeFilter;
    ...
    const handleDelete = async (id: string) => {
        if (!window.confirm("Видалити піхви?")) return;
        try {
            await sheathService.delete(id);
            setSheaths((prev) => prev.filter((s) => s.id !== id));
        } catch (error) {
            console.error("Помилка видалення:", error);
            alert("Не вдалося видалити піхви.");
        }
    };

    const handleToggleActive = async (item: Sheath) => {
        try {
            if (item.isActive) {
                await sheathService.deactivate(item.id);
            } else {
                await sheathService.activate(item.id);
            }
            setSheaths((prev) =>
                prev.map((s) =>
                ...
                <div className="bg-white/80 backdrop-blur-sm rounded-2xl p-6 mb-6 border border-[#b8845f]/20 shadow-sm">
                    <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                        <div className="flex items-center space-x-4">
                            ...
                            <div>
                                <h1 className="text-2xl font-bold text-[#2d3748]">Піхви</h1>
                                <p className="text-[#2d3748]/60">Управління піхвами</p>
                            </div>
                        </div>
                        <button
                            onClick={() => router.push("/sheath/0")}
                                ...
                                >
                                    ...
                                    <td className="p-4 text-center">
                                        <Chip
                                            ...
                                            variant="flat"
                                        >
                                            {item.isActive ? "Активні" : "Неактивні"}
                                        </Chip>
                                    </td>
                                    <td className="p-4">
                                        <div className="flex items-center justify-center space-x-2">
                                            ...
                                            <button
                                                onClick={() => router.push(`/sheath/${item.id}`)}
                                                className="p-2 rounded-lg"
                                                title="Редагувати"
                                            >
                                                <Edit size={16} />
